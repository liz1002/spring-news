		...
		/*try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					bSupportDragAndDropAPI = false;
					console.log("bSupportDragAndDropAPI 1")
				}else{
					bSupportDragAndDropAPI = true;
					console.log("bSupportDragAndDropAPI 2")
				}
			} else {
				bSupportDragAndDropAPI = false;
				console.log("bSupportDragAndDropAPI 3")
			}
		}catch(e){
			bSupportDragAndDropAPI = false;
			console.log("bSupportDragAndDropAPI 4")
		}*/
	}
    ...
    function html5Upload() {	
    	...
    	console.log("html5Upload2 call!!");
    	var tempFile,
    		sUploadURL;
    	
    	/* 변경 하는 곳(프로젝트명 하드코딩) */
    	sUploadURL= "/news/admin/news/";//'file_uploader.jsp';//'file_uploader_html5.php'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		console.log("callFileUploader call!!");
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{ 
 			//http://localhost:8080/news/resources/photo_uploader/popup$%7BpageContext.request.contextPath%7D/admin/upload
 			//sUrl  : location.href.replace(/\/[^\/]*$/, '') + "${pageContext.request.contextPath}/admin/upload",//'/file_uploader.jsp',	//샘플 URL입니다.
 			sUrl  : "/news/admin/news/upload", /* 변경 하는 곳(프로젝트명 하드코딩) */
 			sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 	    	sFiletype : "*.jpeg;*.jpg;*.png;*.bmp;*.gif",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)	
 	    	sMsgNotAllowedExt : 'JPEG,JPG, GIF, PNG, BMP 확장자만 가능합니다',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
